...
CmdParser::openDofile(const string& dof)
{
   ...
   _dofile = new ifstream(dof.c_str());
    ...
    else
    {
        ...
        {
            cerr<<" Reach the max size of _dofileStack "<<endl;
            return false;
        }
    }
...
CmdParser::parseCmd(string& option)
{
   ...
   // TODO...
    string token;
    size_t firstEnd = myStrGetTok( str , token );
...
CmdParser::listCmd(const string& str)
{
   // TODO...
    string token;
    size_t StrEnd = myStrGetTok( str , token );
    if( token.empty() )
    {
        string all;
        CmdExec* e = 0;
        ...
        for( CmdMap::const_iterator iter = _cmdMap.begin() ; iter != _cmdMap.end() ; ++iter )
        {
            ...
            e = iter->second;
            all = first + e->getOptCmd();
            cout<<setw(12)<<left<<all;
            if( ( endline % 5 ) == 4 )
                cout<<endl;
            endline++;
        }
    ...
    {
        vector<CmdMap::const_iterator> check;
        for( CmdMap::const_iterator iter = _cmdMap.begin() ; iter != _cmdMap.end() ; ++iter )
        {
            string all , first = iter->first;
            CmdExec* e = iter->second;
            all = first + e->getOptCmd();
            if( token.size() < all.size() )
            if( myStrNCmp( all , token , token.size() ) == 0 )
        ...
        {
            string all , first = check[0]->first;
            CmdExec*  e = check[0]->second;
            all = first + e->getOptCmd();
            if( StrEnd < str.size() )
            {
                cout<<endl;
            ...
            else if( StrEnd == string::npos )
            {
                string all_sub = all.substr( token.size() );
                for( size_t i = 0 ; i < all_sub.size() ; i++ )
                    insertChar( all_sub[i] );
                insertChar( ' ' );
            }
...
CmdParser::getCmd(string cmd)
{
   ...
   // TODO...
    string token;
    myStrGetTok( cmd , token );
    for( CmdMap::const_iterator iter = _cmdMap.begin() ; iter != _cmdMap.end() ; ++iter )
    {
        string first = iter->first;
        CmdExec* check = iter->second;
        string second = check->getOptCmd();
        string all = first + second;
        if( myStrNCmp( all , token , first.size() ) == 0 )
        {
            e = iter->second;
        }
    }
