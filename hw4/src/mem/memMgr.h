...
#define toSizeT(t)      ( ( t % SIZE_T == 0 ) ? t : ( t + SIZE_T - ( t % SIZE_T ) ) ) // TODO  done
//
// To demote 't' to the nearest multiple of SIZE_T
// e.g. Let SIZE_T = 8;  downtoSizeT(9) = 8, downtoSizeT(100) = 96
#define downtoSizeT(t)  SIZE_T * ( t / SIZE_T )  // TODO  done
...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) {
      // TODO
       size_t Pro_t = toSizeT(t);
       if( Pro_t <= size_t(_end - _ptr) )
       {
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      #endif // MEM_DEBUG
       for( MemBlock<T>* ptr2 = 0 ; _activeBlock->_nextBlock != 0 ; )
       {
           ptr2 = _activeBlock->_nextBlock;
           delete _activeBlock;
           _activeBlock = ptr2;
       }
       _activeBlock->reset();
...
private:
   ...
   size_t getArraySize(size_t t) const {
      ...
      assert(t >= S);
      // TODO
       size_t arrRealBytes = t - SIZE_T;
       size_t ArraySize = arrRealBytes / S;
   ...
   T* getMem(size_t t) {
      ...
      //    #endif // MEM_DEBUG
       size_t prom_t = toSizeT(t);
       if( prom_t > _blockSize)
       {
           cerr << "Requested memory (" << prom_t << ") is greater than block size"
           << "(" << _blockSize << "). " << "Exception raised...\n";
           throw "bad_alloc()";
           ret = reinterpret_cast<T*>( _activeBlock->_ptr );
       }
       size_t n = getArraySize(prom_t);
       ...
       {
           if( _activeBlock->getMem( prom_t , ret ) )
               ;
           else
           {
               ...
               MemBlock<T>* New= new MemBlock<T>( _activeBlock , _blockSize );
               _activeBlock = New;
               _activeBlock->getMem( prom_t , ret );
               #ifdef MEM_DEBUG
               cout << "New MemBlock... " << _activeBlock << endl;
               #endif // MEM_DEBUG
           }
   ...
   size_t getNumBlocks() const {
      // TODO
       size_t num = 0;
       for( MemBlock<T>* ptr = _activeBlock ; ptr != 0 ; ptr = ptr->_nextBlock )
           num++;
      return num;
   }
    // done
